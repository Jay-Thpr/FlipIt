...
async def test_buy_search_agent_retries_once_and_completes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_run_agent_task(agent_slug: str, request: Any) -> AgentTaskResponse:
        ...
        if agent_slug == "ebay_search_agent":
            ...
            output = {
                ...
                "summary": "Prepared 1 negotiation message starting with offerup",
                "offer_messages": [
                    {
                        "platform": "offerup",
    ...
    assert "agent.retrying" in event_types
    retry_event = next(event for event in session.events if event.event_type == "agent.retrying")
    assert retry_event.data == {"agent": "depop_search_agent", "attempt": 2, "max_attempts": 2}
...
async def test_timeout_failure_records_agent_failure_category(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    failure_event = next(event for event in session.events if event.event_type == "agent.failed")
    assert failure_event.data["category"] == "timeout"
    assert failure_event.data["agent"] == "vision_agent"

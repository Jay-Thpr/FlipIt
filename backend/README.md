...
- Pipelines run in the background; `vision_agent` now runs deterministic inference from sell notes and image URL metadata, while the remaining agents still return scaffold outputs.
...
- Replace the remaining stub agent logic with real Browser Use and Gemini logic, starting with the rest of the `SELL` pipeline.

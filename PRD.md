...
## 16. Frontend Data Model (TypeScript Types)

These are the canonical TypeScript types the frontend is built against. Backend writes to Supabase must match these column names.
...
interface Message {
  id: string;
  sender: 'agent' | 'them';   // Backend must write 'sender', not 'role'
  text: string;                // Backend must write 'text', not 'content'
  timestamp: string;
}
```

### 16.4 Completed Trade
```ts
interface Trade {
  id: string;
  name: string;
  type: 'Sold' | 'Bought';
  platform: string;
  price: number;               // Backend must write 'price', not 'final_price'
...
## 18. Frontend Implementation Status

The frontend is **fully built** with mock data (`frontend/data/mockData.ts`). All screens listed in Section 7.3 are implemented and functional. The remaining work is replacing mock data with real Supabase queries and wiring the agent pipeline UI to the FastAPI SSE endpoints.

### 18.1 Frontend Stack (Actual)
- React Native (Expo) with Expo Router
- Pure React Native StyleSheet (no Tailwind/NativeWind in production)
- Lucide icons, react-native-svg for charts
- Custom ThemeContext (Light/Dark/System)
- Supabase client SDK for auth, CRUD, storage, realtime
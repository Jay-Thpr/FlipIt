...
Agent inputs and outputs are now validated against step-specific schemas so pipeline contracts stay structurally stable as real logic is added. `vision_agent`, `ebay_sold_comps_agent`, `pricing_agent`, and `depop_listing_agent` now run deterministic non-stub logic for the `SELL` pipeline.
